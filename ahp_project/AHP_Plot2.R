...
# (within and between groups, mixed design) with or without data points
 lineplotmf <- function(df,idvar,withinvar,groupvar,paramname,xpos,ypos,yticks,ylabels)
	{
	...
	#--------------------------------------------------------
	xticks = c(100,150,200,250,300,350,400,450,500);
	...
	scale_fill_manual("Groups", breaks = levels(factor(dfavgsem$Groups)), values= c("grey","black") )+
	scale_color_manual("Groups", breaks = levels(factor(dfavgsem$Groups)), values= c("black","black") )+
	...
	#coord_cartesian(xlim=c(100,475),ylim=yticks)+
