newline=function (x.factor, response, group = NULL, type = "b", x.cont = FALSE, 
    ...
    xaxt = "s", data = NULL, subset = NULL,bordercol,fillcol, ...) 
{
    ...
    int.plot <- function(x.factor = x.factor, group = group, 
        ...
        matplot(xvals, cells, ..., type = type, xlim = xlim, 
            ...
            xaxt = "n", col = col, lty = lty, lwd = lwd, pch = pch)
        if (axes && xaxt != "n") {
    ...
    if (is.null(group)) {
        ...
        plot(nlevels.x, mn.data, xaxt = "n", type = type, col = col, 
            pch = NA, cex = cex, cex.axis = cex.axis, xlim = if (is.null(xlim)) {
        ...
        if (xaxt != "n") 
            axis(1, label = names(mn.data), at = nlevels.x, cex.axis = cex.axis, 
                ...)
    }
    ...
    if (type %in% c("p", "b")) {
        if (is.null(group)) {
            ...
            points(nlevels.x, mn.data, pch = pch[1], fg = bordercol[i],bg=fillcol[i], 
                cex = cex, col = col)
        }
        else {
            nlevels.y <- dim(mn.data)[2]
            for (i in 1:nlevels.y) points(leg.vals$xvals, mn.data[, 
                i], pch = pch[i], fg = bordercol[i],bg=fillcol[i], col = if (length(col) > 
                1) 
                col[i]
            else col, cex = cex)
        }
